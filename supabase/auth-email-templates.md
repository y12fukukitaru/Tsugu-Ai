...
2. 左のメニューから **「API Keys」**
3. すでにあるキー（`tsugu-reminder`）は使いません。**新しく作ります。**
   理由は2つ。
   - 一覧に出ているのは先頭だけで、**中身は二度と表示されません**（作ったときの一度きり）
   - `tsugu-reminder` は週の便り・毎朝のリマインド・月次レポートが使っています。
     使い回すと、**キーを無効にしたときに全部が同時に止まります**
4. **「+ Create API key」** を押す
   - **Name**：`supabase-auth-smtp`
     （あとで「Last used」を見たとき、どちらが何に使われているか分かります）
   - **Permission**：**Sending access**
     （SMTP はメールを送るだけ。Full access は要りません）
   - **作成直後に表示される文字列をコピーして、メモ帳などに貼っておいてください**（画面を閉じると二度と見られません）

> **`tsugu-reminder` は消さないでください。** Edge Function（週の便りなど）がいまも使っています。
...
**手作業は不要です。** 保存する前、画面の下にこの案内が出ています。

> The email rate limit will be increased to 30 emails per hour after enabling custom SMTP.

**「Save changes」を押した時点で、1時間に30通へ自動的に上がります。**
標準の2〜3通のままだとローンチ日に詰まりますが、その心配はここで消えます。

あとから変えたくなったときだけ、**左メニューの「Rate Limits」**（この設定ページの中ではなく、
Authentication のメニューに並んでいる別項目）から調整できます。

> `Minimum interval per user` は初期値（60秒）のままで結構です。
> 「同じ人に連続して送らない間隔」の設定で、通数の上限とは別のものです。

**ここまでで手順1は完了です。** 登録メールの差出人が `TsuguAi -継-` になり、通数の心配も無くなります。
