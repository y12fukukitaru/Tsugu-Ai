-- =============================================================
-- 顧客が付かないまま宙に浮かない・締結したら知らせる・運営も送れる
-- ---------------------------------------------------------------
--  ■ 招待した人が必ず担当になる
--    claim_my_invite は「招いた人がいまもパートナー（role='consultant'）」で
--    ないと担当を付けなかった。運営（role='admin'）がパートナー表示から
--    招待すると、本人が登録した瞬間に「招いたパートナーが見つかりません」で
--    止まり、担当未割当になっていた。運営も招待できるようにする。
--
--  ■ 運営もパートナー表示から顧問契約を送れる
--    contract_offers の書き込みは role='consultant' に限っていた。運営が
--    自分の顧客を直接伴走するときは、パートナー表示から送れればよい。
--    運営コンソールに同じ送信欄を置く必要はない。
--
--  ■ 締結したら、パートナーと運営に知らせる
--    contract_agree は書面を写し取って終わっていた。同意した瞬間に
--    agent_insights（継ナビくんのお知らせ）を、送った人・担当・運営の全員に
--    入れる。件名に相手のメールアドレス、本文に締結日時（日本時間）と次の一手。
--    「いつ登録したか」は、この締結日時（contract_offers.agreed_at）で分かる。
--
--  確かめかた：招待の関数=1、契約の書き込み=1、締結の知らせ=1
--
...
begin
  ...
  --  招いた人が、いまもパートナーか運営であることを確かめる。
  --  運営が自分の顧客を直接伴走するときは、運営が担当になる
...
-- ---------------------------------------------------------------
--  知らせを入れる。security definer の関数から呼ぶので RLS は通らない。
--  同じ相手に二度入れない（送った人が運営でもあるとき等）
...
--  contract_agree は 20260906010000_contract_ep.sql の定義と同じ。最後に知らせを足しただけ
...
begin
  ...
  --  締結の知らせ。ここで失敗しても同意そのものは通す（書面は残っている）
  begin
