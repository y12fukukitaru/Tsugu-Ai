...
// ② 顧客向け：物語の表と、デモの画面（1年後）が同じ数字
{
  ...
  //  売り手の社長へ：今始める理由（2026-09-24）。例の運送会社を売り手の側から
  ok('顧客向け：譲る側の社長へ・準備した3年が、3年間を数字でのあと・約束することの前',
    PITC.indexOf('data-t="3年間を数字で"') < PITC.indexOf('data-t="譲る側の社長へ"') && PITC.indexOf('data-t="譲る側の社長へ"') < PITC.indexOf('data-t="準備した3年"')
    && PITC.indexOf('data-t="準備した3年"') < PITC.indexOf('data-t="約束すること・しないこと"'));
  ok('売り手：利益100万で値段約500万（×5）', 100 * 5 === 500 && PITC.indexOf('値段は<b>約500万</b>上がる') >= 0);
  ok('売り手：3年で30%引き（継続年数×10%）', 3 * 10 === 30 && PITC.indexOf('3年続けていれば30%引き') >= 0);
  ok('売り手：FA報酬 2,600万×5%＝130万 → 30%引き 91万', 2600 * 0.05 === 130 && Math.round(130 * 0.7) === 91 && PITC.indexOf('<b>91万</b>（顧問3年で30%引き）') >= 0);
  ok('売り手：準備の費用 3.5万×36＋5万＝131万 < 残業代432万', 3.5 * 36 === 126 && 126 + 5 < 432 && PITC.indexOf('月3.5万×36か月＝126万＋初期導入費5万') >= 0 && PITC.indexOf('3年分の顧問料（126万＋5万）') >= 0);
  ok('売り手：売り手プランの額は料金の頁と同じ', PITC.indexOf('売り手プランは<b>月35,000円</b>') >= 0 && PITC.indexOf('売り手プラン（譲渡準備）35,000円') >= 0);
  ok('売り手：結果を約束しない一文', PITC.indexOf('価格・補償・割引の結果をお約束するものではありません') >= 0);
  //  最低報酬は、どなたにもない（LP・特定商取引法の表示どおり）。
  //  顧問先の優遇は「1年以上で継続年数×10%（最大50%）の割引」だけ
  ok('最低報酬は「どなたにもありません」', PITC.indexOf('TsuguAiには、どなたにも最低報酬がありません。') >= 0);
  no('「1年を過ぎると最低報酬がなくなる」と言っていない', /1年を過ぎると最低報酬がなくな|最低報酬があり、割引もない|1年未満なら最低報酬あり/.test(PITC));
  //  物語の順番
...
// ⑤ エンタープライズ（EP-I／EP-II）：同じ1社の物語と、積み上がる収入の数字
{
  ...
  //  EP-I の受取：買い手 36,000・売り手 28,000（80%）、初期導入費 80,000・40,000
  ok('EP-I 10社：受取 344,000・利用料 32,000・差引 312,000',
    8 * 36000 + 2 * 28000 === 344000 && 10 * 2000 + 4 * 3000 === 32000 && E1.indexOf('344,000円') >= 0 && E1.indexOf('<b>312,000円</b>') >= 0);
  ok('EP-I 30社：受取 1,032,000・利用料 75,000・差引 957,000',
    24 * 36000 + 6 * 28000 === 1032000 && 30 * 2000 + 5 * 3000 === 75000 && E1.indexOf('1,032,000円') >= 0 && E1.indexOf('<b>957,000円</b>') >= 0);
  ...
  ok('EP-I 年にすると 約374万・約1,148万', Math.round(312000 * 12 / 10000) === 374 && Math.round(957000 * 12 / 10000) === 1148
    && E1.indexOf('約374万円') >= 0 && E1.indexOf('約1,148万円') >= 0);
  ...
  //  M&A：御社でM&A業務をお持ちなら御社で実行してよい。任された案件だけ運営が受け、5割を御社へ（2026-09-24）
  ok('EP-I：M&Aは御社で実行してよく、任された案件だけ5割', E1.indexOf('M&amp;Aの実行は、<b>御社でM&amp;A業務をお持ちなら御社で</b>') >= 0
    && E1.indexOf('TsuguAiが実行するのは、<b>御社から任せていただいた案件だけ</b>です') >= 0 && E1.indexOf('御社と顧客の契約どおり、<b>御社のもの</b>です') >= 0);
  ok('EP-I：同業向けの3枚（すみわけ・導入してよかった・よくあるご質問）', ['すみわけ', '導入してよかった', 'よくあるご質問'].every(function (t) { return has(E1, t); })
    && E1.indexOf('data-t="部門をつくる"') < E1.indexOf('data-t="すみわけ"') && E1.indexOf('data-t="すみわけ"') < E1.indexOf('data-t="EP-Iとは"')
    && E1.indexOf('data-t="部門の成績表"') < E1.indexOf('data-t="導入してよかった"') && E1.indexOf('data-t="導入してよかった"') < E1.indexOf('data-t="画面で見る"')
    && E1.indexOf('data-t="決めごと"') < E1.indexOf('data-t="よくあるご質問"') && E1.indexOf('data-t="よくあるご質問"') < E1.indexOf('data-t="始め方"'));
  ok('EP-I：Tsugimeの相手でも御社で実行してよい（利益相反の事前説明・ガイドライン）', E1.indexOf('<b>Tsugimeで見つかった相手なら</b>') >= 0
    && E1.indexOf('中小M&amp;Aガイドラインに沿って進めてください') >= 0 && E1.indexOf('個別の扱いは運営と事前に取り決める') < 0
    && R('index.html').indexOf('Tsugimeで見つかった相手との案件も同じです') >= 0 && R('manual-ep.html').indexOf('Tsugimeで見つかった相手との案件も同じ') >= 0);
  no('EP-I：「実行は運営だけ」と言い切る文を残さない', E1.indexOf('買い手候補の特定から先の実行（FA）は<b>TsuguAiの運営</b>が担い') >= 0 || E1.indexOf('部門は<b>仲介・価格交渉・契約実務をしません</b>') >= 0);
  ok('本体・説明書：御社で実行してよい', R('index.html').indexOf('御社で実行していただいて構いません</b>（報酬は御社と顧客の契約どおり') >= 0
    && R('manual-ep.html').indexOf('<b>法人で実行していただいて構いません</b>') >= 0);
  ok('EP-I：看板は仲介ではなく支援', E1.indexOf('看板は「仲介」ではなく<b>「事業承継・M&amp;A支援」</b>') >= 0);
  ok('EP-I：伴走の中で担当者は仲介・価格交渉を一人で抱えない', E1.indexOf('担当者は<b>仲介・価格交渉・契約実務を一人で抱えません</b>') >= 0);
  ok('EP-I：料金は据え置き（立ち上げの別料金なし）', E1.indexOf('立ち上げのための別料金はありません') >= 0
    && /ボリュームディスカウント・EP登録料・月額下限・法人管理料は<b>ありません<\/b>/.test(E1));
  ok('EP-I：成績表の差引は10社の例と同じ', /部門の差引（月）<\/td><td class="up">312,000円/.test(E1));
  ok('本体：資料の説明にも部門の立ち上げ', R('index.html').indexOf('事業承継・M&A支援部門の立ち上げ（すみわけ・組織図・90日・成績表・よくあるご質問）') >= 0);
