...
// ④ 配線
{
  ...
  ok('起動時に読む', /loadAgentInsights\(\); loadSurvey\('customer'\); loadExitPlan\(ME,'customer'\); loadAfterPrep\(ME,'customer'\); knvInit\(\);/.test(SRC) && /loadAgentInsights\(\); loadSurvey\('consultant'\); knvInit\(\);/.test(SRC));
...
// ⑥ SQL・説明書・版
{
  ...
  is('版が揃う', [build, VER.build], ['20260914-01', '20260914-01']);
}
