...
// ④ レベル条件と研修の向き（文言の整合）
// ---------------------------------------------------------------
ok('研修 t1-2 の Lv.3 は「または成約1件」', /lv\(3,'エグゼクティブ','顧問契約5件 または 顧問先の成約1件'/.test(SRC));
ok('研修 t1-2 の Lv.4 は「または成約3件」', /lv\(4,'プレミアム','顧問契約10件 または 成約3件'/.test(SRC));
ok('研修 t1-2 に「成約」の行と場面', /tgRow\('成約　＝ 顧問先が譲り受けてクロージングまで行った件数'/.test(SRC) && /chip:'成約', focus:'agg2'/.test(SRC));
ok('PG_LEVELS の条件と研修の文言が同じ趣旨', /cond:'顧問契約5件、または顧問先の成約1件'/.test(SRC) && /cond:'顧問契約10件、または顧問先の成約3件'/.test(SRC));
...
ok('説明書のレベル条件も「または成約」', /顧問契約5件<br>または成約1件/.test(MANP) && /顧問契約10件<br>または成約3件/.test(MANP));
